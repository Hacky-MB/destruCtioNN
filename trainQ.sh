...
if [ -d /storage ]; then
    # script launched on metacentrum
    metacentrum=0
else
    metacentrum=1
fi
...
do
    ...
    if [ ${metacentrum} ]; then
        cat ${top_dir}/${solver} | sed 's/CPU/GPU/g' > ${top_dir}/tmp
        cat tmp > ${top_dir}/${solver}
        rm ${top_dir}/tmp
    fi

	#train net
	${top_dir}/train2.py -i=200000 -s=${top_dir}/${solver} -v

	if [ $? -ne "0" ]; then
		echo "Error occured during training!" && rm ${top_dir}/tmp
		exit
	else
